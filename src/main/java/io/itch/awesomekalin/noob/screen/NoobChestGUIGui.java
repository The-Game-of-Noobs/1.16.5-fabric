...
package io.itch.awesomekalin.noob.screen;
...
public class NoobChestGUIGui {
	...
	public static class GuiContainerMod extends ScreenHandler implements Supplier<Map<Integer, Slot>> {
		...
		public GuiContainerMod(int id, PlayerInventory inv, Inventory inventory) {
			super(NoobMod.NoobChestGUIScreenType, id);
			this.entity = inv.player;
			this.world = inv.player.world;
			this.inventory = inventory;
			this.customSlots.put(0, this.addSlot(new Slot(this.inventory, 0, 16, 27) {
			}));
			this.customSlots.put(1, this.addSlot(new Slot(this.inventory, 1, 132, 26) {
			}));
			int si;
			int sj;
			for (si = 0; si < 3; ++si)
		...
		public ItemStack transferSlot(PlayerEntity player, int index) {
			...
			if (slot != null && slot.hasStack()) {
				...
				if (index < 2) {
					if (!this.insertItem(itemstack1, 2, this.slots.size(), true)) {
						return ItemStack.EMPTY;
					}
					slot.onQuickTransfer(itemstack1, itemstack);
				} else if (!this.insertItem(itemstack1, 0, 2, false)) {
					if (index < 2 + 27) {
						if (!this.insertItem(itemstack1, 2 + 27, this.slots.size(), true)) {
							return ItemStack.EMPTY;
						}
					} else {
						if (!this.insertItem(itemstack1, 2, 2 + 27, false)) {
							return ItemStack.EMPTY;
						}
					}
					return ItemStack.EMPTY;
		...
		private void slotChanged(int slotid, int ctype, int meta) {
			if (this.world != null && this.world.isClient()) {
				ClientPlayNetworking.send(NoobMod.id("noobchestgui_slot_" + slotid), new GUISlotChangedMessage(slotid, x, y, z, ctype, meta));
			}
		}
	}
